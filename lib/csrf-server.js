...
// const CSRF_TOKEN_EXPIRY = 10; // 10 seconds for testing
...
export async function generateCsrfToken() {
  try {
    ...
    cookieStore.set(CSRF_COOKIE_NAME, jwt, {
      httpOnly: true,
    ...
    console.log("CSRF token generated in cookie and sent to client.");
    return tokenValue;
  } catch (error) {
    ...
    try {
      ...
      if (!token || !(await validateCsrfToken(token))) {
        return Response.json(
          { error: 'CSRF non valide ou expiré. Veuillez actualiser la page et réessayer.' },
          { status: 403 }
        );
      }
