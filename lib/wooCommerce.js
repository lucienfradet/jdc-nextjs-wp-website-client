  ...
  try {
    const res = await axiosInstance.get(`${API_URL}/products`, {
      params: {
        ...
        _fields: "id,name,price,price_html,shipping_class,tax_status,images,description,type,stock_quantity,meta_data,stock_status",
...
export async function fetchBookingProducts() {
  try {
    ...
    // Filter for booking products (type: mwb_booking)
    const bookingProducts = products.filter(product => product.type === 'mwb_booking');

    return bookingProducts;
  } catch (error) {
    return ({ error: `Failed to filter booking products, error: ${error}` }, { status: 500 });
  }
}

export async function fetchBookingProductById(id) {
  try {
    const product = await fetchWooProductById(id);
    
    if (!product) {
      return ({ error: "Booking product not found" }, { status: 404 });
    }

    // Check if this is a booking product
    if (product.type !== 'mwb_booking') {
      return ({ error: "The requested product is not a booking" }, { status: 400 });
    }
