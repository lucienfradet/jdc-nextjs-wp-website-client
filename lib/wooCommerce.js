...
export async function validateOrderData(orderData) {
  try {
    ...
    const serverShippingCost = (() => {
      // Check if shipping calculator is loaded
      // Since we can't directly access the shippingLoaded state, we'll implement 
      // similar logic as in CartContext
      if (!shippingCalculator.hasInitialized) {
      ...
      return shippingCalculator.calculateShipping(validatedItems, deliveryMethod);
    })();
