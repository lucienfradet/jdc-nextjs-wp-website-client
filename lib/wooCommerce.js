  ...
  try {
    const res = await axiosInstance.get(`${API_URL}/products`, {
      params: {
        ...
        _fields: "id,name,price,price_html,shipping_class,tax_status,images,description,short_description,type,stock_quantity,meta_data,stock_status,categories",
...
/**
 * Fetch a specific product by ID and check if it matches the specified filter type
 * @param {number|string} id - Product ID
 * @param {string} filterType - Type of filter to apply: 'booking', 'panier', or 'online-store'
 * @returns {Promise<Object>} - Product object or error object
 */
export async function fetchProductByIdFiltered(id) {
  try {
    const productResponse = await fetchWooProductById(id);

    if (!productResponse.success) {
      return errorResponse(productResponse.error);
    }
    ...
    switch (filterType) {
      case 'booking':
        isValidType = product.type === 'mwb_booking';
        errorMessage = "The requested product is not a booking";
        break;
      case 'panier-de-legumes':
        isValidType = product.categories && 
                     product.categories.some(category => category.slug === 'panier-de-legumes');
        errorMessage = "The requested product is not a panier product";
        break;
      case 'online-store':
        isValidType = product.categories && 
                     product.categories.some(category => category.slug === 'online-store');
        errorMessage = "The requested product is not an online store product";
        break;
      default:
        return errorResponse(`Invalid filter type: ${filterType}`, { status: 400 });
    }
