...
class ShippingCalculator {
  ...
  calculateShipping(cart, deliveryMethod = 'shipping') {
    ...
    cart.forEach(item => {
      // If the item is pickup only, no shipping cost
      if (item.shipping_class === 'only_pickup') {
        return;
      }
      ...
      // Note: Depending on your business rules, you might want to multiply by quantity or not
      // Some shops charge shipping per item, others have flat rates per shipping class
      totalShipping += rate;
    });
