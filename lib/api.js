...
export async function fetchImageData(acfFields) {
  ...
  await processRecursively(processedFields);

  return processedFields;

  // Recursive helper function to process values of any type
  async function processRecursively(obj) {
    ...
    if (Array.isArray(obj)) {
      ...
      await Promise.all(obj.map(async (item, index) => {
        if (typeof item === 'number') {
          // This is potentially an image ID
          obj[index] = await processImageId(item, `array item at index ${index}`);
        } else if (typeof item === 'object' && item !== null) {
          // This is a nested object or array, process it recursively
          await processRecursively(item);
        }
        // For other types, do nothing
      }));
    } else {
      // Process each key-value pair of the object
      await Promise.all(
        Object.entries(obj).map(async ([key, value]) => {
          if (typeof value === 'number') {
            // This is potentially an image ID
            obj[key] = await processImageId(value, `field "${key}"`);
          } else if (typeof value === 'object' && value !== null) {
            // This is a nested object or array, process it recursively
            await processRecursively(value);
          }
          // For other types (including strings with numbers), do nothing
        })
      );
    }
  }

  // Function to process a single image ID
  async function processImageId(value, fieldDescription) {
    const maxRetries = 3;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (!media_details?.sizes) return createFallbackImageData(id, alt_text, source_url, media_details);
        return createImageDataObject(id, alt_text, media_details, source_url);
      } catch (error) {
        if (attempt === maxRetries) {
          console.error(`Error processing ${fieldDescription}:`, error.message);
          return createErrorImageData(value);
        }
