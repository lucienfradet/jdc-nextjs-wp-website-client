...
export async function processProductImages(productData, revalidateTime = 3600) {
  // Handle arrays of products
  if (Array.isArray(productData)) {
    return Promise.all(productData.map(item => processProductImages(item, revalidateTime)));
  }
  ...
  if (!Array.isArray(processedProduct.images)) {
    // Single image ID case
    processedProduct.images = await processImageId(processedProduct.images, "product image", revalidateTime);
  } else {
    // Array of image IDs case
    console.log("processing array");
    processedProduct.images = await Promise.all(
      processedProduct.images.map((img, i) =>
        processImageId(img, `product image at index ${i}`, revalidateTime)
      )
    );
  }
